...
def moving_average(x, w=10):
    ...
    valid = np.isfinite(x) & (x != 0)  # Only consider finite and non-zero values as valid
...
#            data = np.where(totcols>0, np.log10(totcols), -999990.0)
        ...
        # BG: these brantches used often ------------------------------------------------------------------------------------------------------
            ...
            if 'ATM1' in additional_spesifications: title += f' - Atmosphere 1 (ACM-CAP)'
            elif 'ATM0' in additional_spesifications: title += f' - Atmosphere 0 (composite)'
            # title+= ' - Assessment Domain 1x1'
            # title+= ' - Assessment Domain 11x11'
            ...
            # if atmosphere==1: title += f' - Atmosphere 1 (ACM-CAP)'
            # elif atmosphere==0: title += f' - Atmosphere 0 (composite)'
            ...
            RTM_str = 'MYSTIC (3D)' if '3D' in librad_version else 'DISORT (1D)'
            RTM_str = 'DISORT - AOD(x0.4 absorbing)'
            p,=ax.plot(x2, data2, color='blue', label=RTM_str,
        ...
        # BG: made changes to this branch
            ...
            ####### Find max & mean diff + find idx where happen -> find iwp where happen for ice-habit sensitivity ############
            print(f'\n\n Orbit {self.Name} \n Mean Diff: {np.nanmean(data):.2f} W/m²    Max Diff: {np.nanmax(data):.2f} W/m² \n{np.nanmean(data):.1f}     {np.nanmax(data):.1f} \n\n\n')
            idx_max = np.nanargmax(data)
            iwc     = ACMCOM.ice_water_content[:, idx_max]
            heights = ACMCOM.height_layer[:, idx_max]
            mask    = ((heights >= 0) &(heights < 1e35) & (iwc < 1e30))
            valid_heights = heights[mask]
            valid_iwc_at_max_diff = iwc[mask]
            iwp_at_max_diff = -np.trapezoid(valid_iwc_at_max_diff, x=valid_heights)
            print(f'Max Diff at Latitude: {x[idx_max]:.1f}°N, IWP at Max Diff: {iwp_at_max_diff:.2g} g/m²\n\n\n')
            ###############################################################################################################
        ...
        # BG: created by me
            ...
            # -------------------------------
                ...
                # label= '\n(×0.4 absorbing  -  No Aerosols)\n' #'something went wrong' 
                # cblabel = r"$F_{\times 0.4 \, \mathrm{absorbing}} - F_{None}$"  #''   
                label = 'Something Went Wrong'
                clabel = ''
            ...
            print(f'\n\n Orbit {self.Name} \n Mean Diff: {np.nanmean(data):.2f} W/m²    Max Diff: {np.nanmax(np.abs(data)):.2f} W/m² \n{np.nanmean(data):.1f}     {np.nanmax(np.abs(data)):.1f} \n\n\n')
    ...
    def PlotAllLevels(self, plot_type, pngfile, Scene2, ACMCOM):
        ...
        for i, (ax, data, cblabel, label) in enumerate(zip(ax_list, data_list, cblabel_list, label_list)):
            ...
            if quantity_list:
                ...
                if add_profile_list: 
                    ax2.legend(handles=add_profile_list, 
                                                loc='upper right', framealpha=0.7, 
                                                borderaxespad=0.0,                  # space to axes
                                                borderpad=0.25, labelspacing=0.25,   # compact box)
                                                fontsize=INFOSIZE*.8)
            ...
            if 'ax2' in locals() and hasattr(fig, '_ax_bot'):
                ax2.set_xlabel(xlabel + xlabel_specs, fontsize=INFOSIZE)
...
if __name__ == "__main__":
    ...
    AssDomainSize       = [(11,11), (3,3), (1,1)][0]   # Size of assessment domain (across-track, along-track) in libRadtran 3D MC
    ...
    want_info           = True                 # Prints out SZA, Prints out CF
    ...
    # additional_spesifications =  '_RA'
            ...
            # Test ATM 0 
    # additional_spesifications += '_21x21_ATM0'
    # additional_spesifications += '_ATM0'
    # additional_spesifications += '_ATM0_for_Qu'
    # additional_spesifications += '_ATM1_for_Qu'
    ...
    # additional_spesifications2 = '_GHM'
    # additional_spesifications2 = '_AOD(NONE)'
    # additional_spesifications2 = '_AllLevels_edn_edir'
    # additional_spesifications2 = '_AllLevels_eup'

    # additional_spesifications2 = '_ATM0'
