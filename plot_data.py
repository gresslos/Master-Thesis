...
def plot(versions, orbits, places, values, stds, ylabel, title, show_stds=True, png_name="Data/plot.png"):
    ...
    fig.suptitle(title, fontsize=FONTSIZE, y=0.94)
...
def get_instances(SceneName, Product, Product2=False, BMAFLX=False):
    ...
    else: info = (np.nan, np.nan, baseline_str)
...
if __name__ == "__main__":
    ...
    if what_to_plot == 'Ice_habit':
        ...
        titles      =  ["Ice Habit Sensitivity\nSW TOA Mean Flux Difference (DISORT − BMA-FLX)",
                        "Ice Habit Sensitivity\nLW TOA Mean Flux Difference (DISORT − BMA-FLX)"]

        ##################################
        want_average_line           = False
        want_quality_status         = False

        Product2  = True  # TEST
        additional_spesifications2 = '_GHM'
        versions  = ['SC - GHM', 'RA - GHM']
        additional_spesifications  = ['_SC', '_RA']
        librad_version2 = librad_version

        # titles      =  ["SW TOA Flux Difference (swath-averaged)",
        #                 "LW TOA Flux Difference (swath-averaged)"]
        titles      =  ["SW\n",
                        "LW\n"]
        ##################################
    ...
    #######################################################################################################################
        ...
        titles = ["Aerosol Sensitivity\nLW TOA flux difference (DISORT − BMA-FLX)"]
    ...
    #######################################################################################################################
        ...
        idx_scene = [3,4,5,6,7,8,11,12] #  [3,4,5,6,7,8,11,12] # [5,8,12] #[3,4,5,6,7,8]
        sources   = [['solar'],['thermal']][1] # Chose either solar or thermal, not both
        if sources[0] == 'thermal': source_str = 'thermal' # New thermal runs only run in thermal (new surface temp.)
        versions = sources
        ...
        additional_spesifications   = [
                                    [''],['_21x21'],
                                    ['_GHM'],['_SC'],['_RA'],
                                    ['_AOD(NONE)'],['_AOD(default)'],['_AOD(abs0.6)'],['_AOD(abs0.4)'],['_AOD(abs0.2)'],['_AOD(0.6)'],['_AOD(0.4)'],['_AOD(0.2)']
                                    ][idx]
        titles                      = [
                                    [' - DISORT'],[' - MYSTIC'],
                                    [' - GHM'],[' - SC'],[' - RA'],
                                    [' - AOD(NONE)'],[' - AOD(default)'],[' - AOD(abs0.6)'],[' - AOD(abs0.4)'],[' - AOD(abs0.2)'],[' - AOD(0.6)'],[' - AOD(0.4)'],[' - AOD(0.2)']
                                    ][idx]

        if 'AOD' in additional_spesifications[0]: source_str = 'thermal'
        # titles = ['']
