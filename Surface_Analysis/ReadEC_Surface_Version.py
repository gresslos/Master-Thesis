...
class Scene:
    ...
    def ExtractData(self, BMAFLX, ia, shape=(1,1), idx_scene=None):
        ...
        if 'TOA' in self.fn: 
            ...
            pyranometer_data = np.loadtxt('SUR/pyranometer_fluxes.txt')
    ...
    def plot_all_levels(self, Scene2=None, idx_scene=None, ia=None):
        ...
        ax.minorticks_on()
         ...
         # if plot_type == 'all_levels_solar':
                                                                                ...
                                                                                #     add_profile_list = [] # List for ax2-legends
                                                                                #     for quantity in quantity_list:
                                                                                #         ax, ax2 = add_profile_to_plot(fig, ax, ACMCOM, fsize=FONTSIZE, legend_list=add_profile_list, quantity=quantity, quantity_list=quantity_list, stacked=True, name=self.Name, data=data)
                                                                                #     if add_profile_list: ax2.legend(handles=add_profile_list, 
                                                                                #                                     loc='upper right', framealpha=0.7, 
                                                                                #                                     borderaxespad=0.0,                  # space to axes
                                                                                #                                     borderpad=0.25, labelspacing=0.25,   # compact box)
                                                                                ...
                                                                                # ##### Add Station Point on Figure #####
                                                                                # ax.plot(x[ia], 0.1, 'ro', markersize=5, zorder=20, label='Observation site')
                                                                                # #######################################
                                                                                ...
                                                                                #     N = len(x)
                                                                                #     half_N = int(N/2)
                                                                                #     if np.nanmax(data[:, :half_N]) > np.nanmax(data[:, half_N:]):
                                                                                #         x = x[:half_N] 
                                                                                #         z = z[:, :half_N]
                                                                                #         data = data[:, :half_N]
                                                                                #         cloud = cloud[:, :half_N]
                                                                                ...
                                                                                #     Z = z[:cloud.shape[0], :]    
                                                                                #     size = 10 if want_closeup else 1
                                                                                ...
                                                                                # h_max = np.nanmax(z[cloud]) + 2      #### IMPORTANT NOTE: do not calculate Fluxes above around 40 km!
                                                                                ...
                                                                                # pngfile = (
                                                                                #     plotdir_base + SceneName + '_' + librad_version + '_' + plot_type + '.png'
                                                                                #     if not want_closeup else
                                                                                #     plotdir_base + SceneName + '_' + librad_version + '_' + plot_type + '_Closeup.png'
                                                                                # )
                                                                                # print("pngfile", pngfile)
                                                                                # plt.savefig(pngfile)
                                                                                # plt.close()
...
if __name__ == "__main__":
    ...
    SceneNames = ['Orbit_01628D','Orbit_01488C','Orbit_05767C','Orbit_06545C','Orbit_06934C',#4
            'Orbit_07323C','Orbit_07712C','Orbit_01301D','Orbit_02437D','Orbit_02826D','Orbit_03215D',#10
            'Orbit_03604D','Orbit_03993D','Orbit_04382D','Orbit_05160D','Orbit_05549D','Orbit_05938D',#16
            'Orbit_07883D','Orbit_01628D','Orbit_01690C','Orbit_01239C','Orbit_01768C','Orbit_02546C',#22
            'Orbit_02935C','Orbit_04102C','Orbit_04491C','Orbit_04880C','Orbit_05269C','Orbit_05658C',#28
            'Orbit_06047C','Orbit_01643C','Orbit_01629C','Orbit_02826D','Orbit_03215D','Orbit_03993D',#34
            'Orbit_05160D','Orbit_07883D','Orbit_01643C','Orbit_02452C','Orbit_03619C','Orbit_04008C',#40
            'Orbit_05175C','Orbit_07898C','Orbit_01690D','Orbit_01690D','Orbit_01161C','Orbit_01830C',
            'Orbit_02608C','Orbit_02997C','Orbit_03386C','Orbit_03775C','Orbit_04164C','Orbit_04942C',
            'Orbit_05331C','Orbit_05720C','Orbit_06498C','Orbit_06887C','Orbit_07276C','Orbit_07665C',
            'Orbit_01270D','Orbit_00959D','Orbit_01643C']   # 'Orbit_06311C']
    station_list = [
        "Bergen",
        "Bjornoya", "Bjornoya", "Bjornoya", "Bjornoya", "Bjornoya", "Bjornoya",
        "Blindern",
        "Filefjell", "Filefjell", "Filefjell", "Filefjell", "Filefjell", "Filefjell",
        "Filefjell", "Filefjell", "Filefjell", "Filefjell",
        "Flesland",
        "Gratangen",
        "Hopen", "Hopen", "Hopen", "Hopen", "Hopen", "Hopen", "Hopen", "Hopen", "Hopen", "Hopen",
        "Iskoras",
        "Jan_Mayen",
        "Juvvasshoe", "Juvvasshoe", "Juvvasshoe", "Juvvasshoe", "Juvvasshoe",
        "Karasjok",
        "Korgasen", "Korgasen", "Korgasen", "Korgasen", "Korgasen",
        "Osteras",
        "Rena",
        "Tromso", "Tromso", "Tromso", "Tromso", "Tromso", "Tromso", "Tromso",
        "Tromso", "Tromso", "Tromso", "Tromso", "Tromso", "Tromso", "Tromso",
        "Trondheim",
        "Vaernes",
        "Karasjok"]
    ial_list = [
        901, 3926, 3923, 3923, 3923, 3922, 3923, 951, 813, 811,
        812, 812, 813, 813, 813, 813, 813, 812, 912, 4606,
        3687, 3681, 3680, 3680, 3679, 3680, 3681, 3680, 3680, 3680,
        4551, 4348, 755, 757, 758, 758, 757, 4532, 4468, 4467,
        4467, 4468, 4465, 949, 789, 4507, 4497, 4499, 4499, 4497,
        4498, 4498, 4499, 4499, 4498, 4499, 4499, 4498, 4498, 561,
        557,4516]
    if WANT_SUR: 
        iacr_list = [
        151, 145, 145, 146, 145, 145, 148, 153, 154, 145,
        151, 153, 152, 155, 153, 155, 154, 149, 154, 152,
        155, 155, 152, 154, 152, 153, 155, 154, 155, 155,
        152, 146, 148, 153, 154, 155, 152, 153, 152, 151,
        151, 152, 150, 151, 148, 150, 150, 150, 149, 146,
        146, 146, 148, 151, 148, 150, 151, 151, 151, 149,
        149, 140] 
    else: iacr_list = np.ones(62)*151 # Compare against nadir-pixel (as BBR see)

                                       
              
    # idx_scenes = np.arange(0,len(station_list))
    idx_scenes = [19]

    SceneNames      = [SceneNames[i] for i in idx_scenes]
    station_list    = [station_list[i] for i in idx_scenes]
    ial_list        = [ial_list[i] for i in idx_scenes]
    ...
    rte_specs = 'MYSTIC_' if want_3D else 'DISORT_'
    obs_level = 'SUR/' if WANT_SUR else 'TOA/'
    out_file_name1 = obs_level + rte_specs + "output_fluxes_NEW.txt"
    out_file_name2 = obs_level + rte_specs + "output_fluxes_INFO.txt"
    ...
    with open(out_file_name1, "w") as flux_file, open(out_file_name2, "w") as info_file:
        ...
        for SceneName, station, ia, iacr, idx_scene in zip(SceneNames, station_list, ial_list, iacr_list, idx_scenes):
            if source_str:
                Product = (
                    'libRad_' + 
            ...
            pathL2TestProducts += f'{station}'

            Product ='ALL_3D_'#'ACM_COM' #'ACM_3D_'#'ACM_COM' #
            # ProductPath = 'ECA_EXAB_'+Product+'*'  # 'ECA_EXAA_'+Product+'*'
            ProductPath = '*'+Product+'*' 
            ProductFile = os.path.join(pathL2TestProducts, SceneName, ProductPath, '*'+Product+'*.h5')
            ProductFile = sorted(glob.glob(ProductFile))[0]
            ...
            if 'ExtractData' in ToDo:
                libRad.ExtractData(BMAFLX, ia=ia, shape=AssDomainSize, idx_scene=idx_scene)


            if 'plot_all_levels' in ToDo:
                libRad.plot_all_levels(libRad2, idx_scene, ia=ia)
