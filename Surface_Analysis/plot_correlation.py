...
def read_flux(filename):
    ...
    flux = []
    for line in lines:
        if '-' in line or '0.00' in line or 'nan' in line:
            flux.append(np.nan)
        else:
...
zout = 'SUR' if want_SUR else 'TOA' 
AssDomain = ['1x1', '3x3', '11x11'][0] 
filename_libRad_fluxes = [f'{zout}/MYSTIC_output_fluxes{AssDomain}.txt', f'{zout}/DISORT_output_fluxes.txt']
        # rte_solver = 'MYSTIC' if want_3D else 'DISORT'
        # filename_libRad_flux = zout + '/' + rte_solver + '_output_fluxes.txt'
filename_observed_flux = 'SUR/pyranometer_fluxes.txt' if want_SUR else 'TOA/BMAFLX_solar_full_quality_flag0,2.txt' # 'TOA/BMAFLX_solar_without_quality_status.txt' 
# filename_observed_flux = 'SUR/pyranometer_TOA_SUR_comparison.txt' if want_SUR else 'TOA/BMAFLX_TOA_SUR_comparison.txt'
pngfile = 'correlation_@' + zout + '.png'                                                     # without_quality_status -> 29 products compare
                                                                                                            # with quality status    -> 23 products compare
...
# title =  f'Surface Downward SW Flux - RTMs vs Ground-Based Observations' if want_SUR else f'TOA Upward SW Flux - RTMs vs BMA-FLX'
# title =  f'Surface Downwelling SW Flux - {rte_solver} vs Ground-Based Observations' if want_SUR else f'TOA Upward SW Flux - {rte_solver} vs BMA-FLX'
pl_list = []


# Extract data to plot
for filename_libRad in filename_libRad_fluxes:
    color = 'mediumseagreen' if 'DISORT' in filename_libRad else 'blue'
    rte_solver = 'DISORT (1D)' if 'DISORT' in filename_libRad else 'MYSTIC (3D)'

    x = read_flux(filename=filename_observed_flux) # model truth
    y = read_flux(filename=filename_libRad)
    print("Before filtering ", len(x), '                      ', filename_libRad)

    print(len(x), len(y), filename_libRad)
    ...
    p, = ax.plot(xx, a + b * xx, lw=1.5, c=color, label=f"{rte_solver}  y = {a:.2f} + {b:.2f}x") # WRITE IN FIG-TEXT: Linear Least Squares (fit)") 
    pl_list.append(p)
    ...
    else: ax.scatter(x, y, s=14, alpha=.8, c=color) # No color-mapping


    bias_str = r"$\Delta F_{\mathrm{SUR}}^{\uparrow}$ (Bias) = " if want_SUR else r"$\Delta F_{\mathrm{TOA}}^{\uparrow}$ (Bias) = "
    data_str = (
        rf"         $\mathbf{{{rte_solver}}}$" + "                \n"
        # f"n = {x.size}\n"
        f"R² = {r2:.2f}                   \n"
        # f"RMSE = {rmse:.1f}\n"
        + bias_str + f"{bias:.1f}" + r" $\pm$ " + f"{std:.1f}" + r" W/m$^2$"
        # f"\nMAE = {mae:.1f}\n"
    )
    fig.text(.98, 0.12 if '1D' in rte_solver else 0.01, 
