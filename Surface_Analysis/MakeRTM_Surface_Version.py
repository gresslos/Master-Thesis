...
import ReadEC_Surface_Version as ReadEC
...
if __name__ == "__main__":
    ...
    ##############################################################################################
    WANT_SUR = False         # Want zout at SUR or TOA
    ...
    # print(len(SceneNames), len(station_list), len(ial_list), len(iacr_list))
            
    # idx_scene = np.arange(0,len(station_list))
    # idx_scene = np.arange(len(station_list)-8,len(station_list))
    # idx_scene = np.arange(55,57)
    idx_scene = [19]

    SceneNames      = [SceneNames[i] for i in idx_scene]
    station_list    = [station_list[i] for i in idx_scene]
    ial_list        = [ial_list[i] for i in idx_scene]
    iacr_list       = [iacr_list[i] for i in idx_scene]
    ...
    # --------------------------------------------------------------------------------------------
                                                ...
                                                #     # rte_specs = 'MYSTIC_TEST_NEW_IA_ON_SURFACE_'
                                                # out_file_name1 = obs_level + rte_specs + "output_fluxes.txt"
                                                # out_file_name2 = obs_level + rte_specs + "output_fluxes_INFO.txt"
                                                # # out_file_name3 = obs_level + "BMAFLX_solar.txt"
                                                # with open(out_file_name1, "w") as flux_file, \
                                                #      open(out_file_name2, "w") as info_file:
                                                #     #  open(out_file_name3, "w") as BMAFLX_file:
                                                    
                                                #     info_file.write(f'{'SUR' if WANT_SUR else 'TOA'} {rte_solver} {RTdimension} {buffer_str}\n'
                                                #     'SceneName ial,iacr Source Flux (Product) \n'
                                                #     '-----------------------------------\n')


    # Start processing (generate and read EartCare input files)
    for SceneName, station, iacr, ial in zip(SceneNames, station_list, iacr_list, ial_list):
        iacr = int(iacr)
        ...
        if 'AllLevels' in additional_spesifications:
            ...
            # THIS IS FOR ORBIT_01690C (idx 19)
            ialongs = np.arange(3800,4750)
                    ...
                    for source in sources:
                        try: 
                            ...
                            if 'AllLevels' in additional_spesifications:
                                ...
                                print(
                                    f"--------------------------- ia={ia} --------------------------\n"
                                    f"  Mean flux ({source}) +- std = {e_str} {std_str} W/m2\n"
                                    "--------------------------------------------------------------\n"
                                )
                            ###################################
